...
def rest_yt_search():
    ...
    return json.dumps(this_rest, indent=4, sort_keys=True)
@app.route('/api/ytlist',methods=['POST','GET'])
def rest_listYt():
    this_query = request.args['query']
    this_rest = scrap.listYt(this_query)
    return json.dumps(this_rest, indent=4, sort_keys=True)
...
def rest_newsSindo():
	...
	this_rest = scrap.newsSindo(this_query)
	return json.dumps(this_rest, indent=4, sort_keys=True)
@app.route('/api/simi',methods=['POST','GET'])
def rest_sendSimisimi():
...
@app.route('/api/smuledl1',methods=['POST','GET'])
def rest_smule():
    this_query = request.args['url']
